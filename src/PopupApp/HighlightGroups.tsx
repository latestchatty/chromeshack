...
const HighlightGroup = memo((props: { name: string }) => {
  ...
  const [localGroup, setLocalGroup] = useState<HighlightGroup>(
    state.highlightgroups?.find(
      (g) => g.name.toUpperCase() === name.toUpperCase()
    )
  );
  ...
  const setInput = (
    ...
    const fieldVal =
      fieldKey === "enabled"
        ? (_this as HTMLInputElement)?.checked
        : _this?.value;
  ...
  useEffect(() => {
    const _group = state.highlightgroups?.find(
      (g) => g.name.toUpperCase() === name.toUpperCase()
    );
    const newLocalGroup = {
      ..._group,
  ...
  return (
    <div id="highlight_group">
      ...
      <div className="group_css">
        ...
        <div
          className="test_css"
          title="Click to try a new style"
          onClick={handleSplotchClick}
        >
          <span
            id={`${trimName(nameInput)}_splotch`}
            title="Click to try a new color"
          >
            Aa
          </span>
        </div>
  ...
  return (
    <div id="highlight_groups">
      {arrHas(state.highlightgroups) &&
        state.highlightgroups?.map((g, i) => (
          <HighlightGroup key={i} name={g.name} />
        ))}
