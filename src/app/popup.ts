...
const randomHsl = () =>
  `hsla(${getRandomNum(0, 360)}, ${getRandomNum(25, 100)}%, ${getRandomNum(
    35,
    60
  )}%, 1)`;
...
const getRandomNum = (min, max, precision?: number) =>
  parseFloat(
    (Math.random() * (max - min) + min).toPrecision(precision ? precision : 1)
  );
...
const getHighlightGroups = async () => {
  ...
  if (highlightRecords.length > 0)
    return await settings.setSetting("highlight_groups", highlightRecords);
};

const getHighlightGroup = (groupElem?) => {
  // serialize a highlight group
  if (groupElem) {
    let enabled = groupElem.querySelector(
      ".group_header input[type='checkbox']"
    ).checked;
    let name = groupElem.querySelector(".group_label").value;
    let built_in = groupElem
      .querySelector(".group_label")
      .hasAttribute("readonly");
    let css = groupElem.querySelector(".group_css textarea").value;
    let users = [
      ...groupElem.querySelector(".group_select select").options,
    ].map((x) => x.text);
...
const delayedTextUpdate = (e) => {
  ...
  debouncedUpdate = setTimeout(async () => {
    ...
    if (updatedCSSField.value.length > 0)
      cssSplotch.setAttribute("style", common.superTrim(updatedCSSField.value));
...
const addHighlightGroup = (e: Event, group?) => {
  ...
  groupElem.innerHTML = `
        ...
        <div class="group_css">
            ...
            <div class="test_css">
                <span id="${trimmedName}_splotch" style="${
    group.css
  }" title="Click to try a new color">Aa</span>
            </div>
        </div>
    `;
  if (group.built_in) {
    ...
    groupElem
      .querySelector("#remove_group")
      .setAttribute("style", "display: none;");
    groupElem
      .querySelector(".group_select")
      .setAttribute("style", "display: none;");
    groupElem
      .querySelector(".group_option_input")
      .setAttribute("style", "display: none;");
  }
  ...
  groupElem
    .querySelector("#option_add")
    .addEventListener("click", async (e) => {
      const this_node = <HTMLElement>e.target;
      const this_input = this_node.parentNode.parentNode;
      let optionContainer = this_node.parentNode.parentNode.querySelector(
        ".group_select > select"
      );
      let username = this_node.parentNode.querySelector(
        ".group_option_textinput"
      );
      let groupName = (<HTMLInputElement>(
        this_input.querySelector(".group_label")
      )).value;
      let this_username = (<HTMLInputElement>username).value;
      let groupHas = await settings.highlightGroupContains(
        groupName,
        this_username
      );
      if (groupHas) {
  ...
  groupElem
    .querySelector("#option_del")
    .addEventListener("click", async (e) => {
      let groupElem = (<HTMLElement>e.target).closest("#highlight_group");
      let groupName = (<HTMLInputElement>(
        groupElem.querySelector(".group_label")
      )).value;
  ...
  groupElem.querySelector(".test_css span").addEventListener("click", (e) => {
    ...
    let firstColor;
    let style = (<HTMLInputElement>styleField).value.replace(
      /((?:\s*?)color:.+?;)/gim,
      (m, g1) => {
    ...
    (<HTMLInputElement>styleField).value = common.superTrim(
      `${style} color: ${randomHsl()} !important;`
    );
    delayedTextUpdate(e); // fire off a css field update
  });
  // handle changes on mutable text fields with a debounce
  let textfields = [
    ...groupElem.querySelectorAll(".group_css textarea, .group_label"),
  ];
  ...
  if (e && (<HTMLElement>e.target).matches("#add_highlight_group"))
    getHighlightGroups();
};
...
const filterOptionsChanged = (e) => {
  ...
  let selected = (<HTMLSelectElement>document.getElementById("filtered_users"))
...
const getUserFilters = async () => {
  ...
  let fullpostHider = <HTMLInputElement>(
    document.getElementById("cuf_hide_fullposts")
  );
  if (fullpostHider)
    await settings.setSetting(fullpostHider.id, fullpostHider.checked);
  return await settings.setSetting("user_filters", users);
};

const addUserFilter = async (e) => {
  let username = <HTMLInputElement>(
    document.getElementById("new_user_filter_text")
  );
  let usersLst = document.getElementById("filtered_users");
...
const getEnabledScripts = async () => {
  ...
  for (let checkbox of checkboxes) {
    ...
    if (
      common.elementMatches(_checkbox, ".script_check") &&
      _checkbox.checked
    ) {
  ...
  for (let script of scripts) {
    for (let checkbox of checkboxes) {
      ...
      if (common.elementMatches(checkbox, ".script_check")) {
        ...
        let settingsChild = document.querySelector(
          `div#${_checkbox.id}_settings`
        );
        if (_checkbox.checked && settingsChild)
          settingsChild.classList.remove("hidden");
        else if (!_checkbox.checked && settingsChild)
...
const clearSettings = (e) =>
  settings
    ...
    .then(saveOptions(e))
    .then(() => {
      let settings_field = <HTMLInputElement>(
        document.getElementById("import_export_field")
      );
  ...
  if (settingsField) {
    settings.getSettings().then(async (settings) => {
      ...
      const sanitizedGroups =
        settings.highlight_groups.filter((x) => !x.built_in) || [];
      const sanitizedSettings = common.objConditionalFilter(
        disallowed,
        settings
      );
      const exportable =
...
const importSettings = (settingsField) => {
  try {
    let parsedSettings =
      settingsField && JSON.parse(common.superTrim(settingsField.value));
...
const handleImportExportSettings = () => {
  ...
  let importExportField = <HTMLInputElement>(
    document.getElementById("import_export_field")
  );
...
const parseSettingsString = (string) => {
  ...
  try {
    let parsed =
      string && string.length > 0 && JSON.parse(common.superTrim(string));
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  trackChanges();
  document
    .getElementById("add_highlight_group")
    .addEventListener("click", addHighlightGroup);
  document
    .getElementById("import_export_field")
    .addEventListener("input", handleImportExportField);
  document
    .getElementById("import_export_btn")
    .addEventListener("click", handleImportExportSettings);
  document
    .getElementById("clear_settings")
