...
const VideoLoader = {
    ...
    getVideoType(url) {
        ...
        let _isXboxDVR = /https?:\/\/(?:.*\.)?xboxdvr\.com\/gamer\/([\w-]+)\/video\/([\w-]+)/i.exec(
            url
        );
        ...
        let _isMixer = /https:\/\/(?:.+\.)?mixer\.com\/([\w-]+)(\?vod=[\w-]+|\?clip=[\w-]+)?/i.exec(
            url
        );
        ...
        if (_isYoutube || _isYoutubeShort) {
            ...
            // twitch clips
                ...
                return { type: 2, clip: _isTwitch[1] || _isTwitch[2] };
        }
    ...
    toggleVideo(e, videoObj, postId, index) {
        ...
        if (e.button == 0) {
            ...
            if (videoObj && videoObj.type === 1)
                VideoLoader.createYoutube(link, videoObj, postId, index);
            else if (videoObj && videoObj.type === 2)
                VideoLoader.createTwitch(link, videoObj, postId, index);
