...
export const CommentTags = {
	...
	async cacheInjectables() {
		const setToggled = (await getSetting(
			"tags_legend_toggled",
			false,
		)) as boolean;
		const table = parseToElement(/*html*/ `
            <div id="post_sub_container">
                <div id="shacktags_legend">
                    ...
                    <table id="shacktags_legend_table" class="${
											!setToggled ? "hidden" : ""
										}">
		...
		for (const tr of CommentTags.tags) {
			...
			for (const tag of tr) {
				const [name, opening_tag, closing_tag, class_name, clickFuncAsString] =
					tag || [];
				const name_td = row.appendChild(document.createElement("td"));
				const span = parseToElement(
					/*html*/ `<span class="${class_name}">${name}</span>`,
				);
				name_td.appendChild(span);
				if (clickFuncAsString?.length > 0)
					name_td.setAttribute("onclick", clickFuncAsString);
