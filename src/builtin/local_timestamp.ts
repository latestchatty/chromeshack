...
export const LocalTimeStamp = {
  ...
  fixTime(rawDateStr: string) {
    ...
    try {
      const fixAMPM = rawDateStr.replace(
        /(am\s|pm\s)/,
        (m1) => ` ${m1.toUpperCase()}`
      );
      LocalTimeStamp.date.setTime(Date.parse(fixAMPM));
      // use native JS to format a date string
      const toStr = LocalTimeStamp.date.toLocaleDateString(
        "en",
        LocalTimeStamp.dateOpts
      );
  ...
  replaceTime(dateStr: string, postDate: HTMLElement) {
    ...
    if (timestamp?.nodeType === 3)
      timestamp.parentNode.replaceChild(fixedTime, timestamp);
  },

  adjustPostTime({ post }: PostEventArgs) {
    const dateStr =
      post?.childNodes?.[2]?.textContent || post?.childNodes?.[1]?.textContent;
