...
export const Collapse = {
  ...
  async cullAfterCollapseTime() {
    const filtered = Collapse.collapsed.filter(
      (c) => !timeOverThresh(c.timestamp, Collapse.timeout)
    );
  ...
  async toggle(args: PostEventArgs) {
    ...
    if (post && is_root) {
      ...
      if (found) {
        ...
        collapsedPostEvent.raise({ threadid: rootid, is_collapsed: false });
    }
  },
