...
export const ModBanners = {
  ...
  install() {
    ...
    if (!rootContainer?.classList?.contains("show_banners"))
  ...
  processPost(args: PostEventArgs) {
    ...
    if (ModBanners.isEnabled && post) {
      const _isOfftopic = post.querySelector(
        "li > div.fullpost.fpmod_offtopic:not(.getPost)"
      ) as HTMLElement;
      const _isStupid = post.querySelector(
        "li > div.fullpost.fpmod_stupid:not(.getPost)"
      ) as HTMLElement;
      const _isPolitical = post.querySelector(
        "li > div.fullpost.fpmod_political:not(.getPost)"
      ) as HTMLElement;
      ...
      if (_isOfftopic) {
        ...
        const _url = `url("${chrome.runtime.getURL(
