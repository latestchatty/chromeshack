...
export const EmojiPoster = {
	...
	apply(args: PostboxEventArgs) {
		...
		const _postBtn = postbox?.querySelector(
			"button#frm_submit",
		...
		if (!_postBtn?.hasAttribute("cloned")) {
			...
			const handleSubmit = (e: MouseEvent | Event) => {
				...
				const _postBox = document.getElementById(
					"frm_body",
				) as HTMLInputElement;
	...
	handleSubmit(postText: string) {
		...
		if (
			...
			$("body").trigger("chatty-new-post-reply", [
				$submitBtn.closest("div.root > ul > li").first().attr("id"),
			]);
			return false;
		}
	...
	handleEncoding(text: string) {
		...
		const escapeCP = (codePoint: number) =>
