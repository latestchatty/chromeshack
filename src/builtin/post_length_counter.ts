...
export const PostLengthCounter = {
	...
	apply(args: PostboxEventArgs) {
		...
		if (postbox?.querySelector("#post_length_counter_text") || !position)
			return;
	...
	update(postbox: HTMLElement) {
		const counter = postbox?.querySelector(
			"#post_length_counter_text",
		) as HTMLElement;
		const textarea = postbox?.querySelector(
			"textarea#frm_body",
		) as HTMLInputElement;
		...
		const astralCount =
			encodedText && EmojiPoster.countAstrals(encodedText).astralsCount;
		...
		if (counter)
			counter.textContent =
				charCount > PostLengthCounter.MAX_POST_BYTES
					...
					: `Characters remaining in post preview: ${
							PostLengthCounter.MAX_POST_BYTES - charCount
					  }`;
	},
};
