...
const tabReducer = (state: UploaderState, action: UploaderAction) => {
	...
	const actionHasUrl =
		urlAction?.payload?.length >= 13 ? urlAction.payload : undefined;
	const stateHasFiles = state.fileData.length > 0 ? state.fileData : undefined;
	const actionHasFiles =
		fileAction?.payload?.length > 0
			? packValidTypes(state.formats, fileAction.payload)
			: undefined;

	const commonValidState = {
		filesDisabled: true,
		uploadDisabled: false,
		cancelDisabled: false,
	};
	const commonInvalidState = {
		filesDisabled: false,
		uploadDisabled: true,
		cancelDisabled: true,
		urlData: "",
	...
	switch (action.type) {
		case "LOAD_TAB":
			if (nextTabName === "IMGURTAB") {
				...
				return nextTabName === "IMGURTAB"
					? { ...newState, multifile: true }
					: newState;
			}
			break;
