...
const checkReplyCeiling = (rootEl: HTMLElement) => {
  ...
  const totalReplies =
    (!window.chrome && replies?.length < 333) || replies.length < 750;
...
const asyncResolveTags = (post: HTMLElement, timeout?: number) => {
  const removeUnusedTagline = async (p: HTMLElement) => {
    ...
    const rootTags = p?.querySelectorAll(
      ".root>ul>li > .fullpost span.lol-tags"
    );
  ...
  const collectTagData = (p: HTMLElement) => {
    const postTags = [
      ...(p?.querySelectorAll(
        ".fullpost > .postmeta .lol-tags > .tag-container"
      ) ?? []),
    ] as HTMLElement[];
...
export const handlePostRefresh = async (
  args: PostEventArgs,
  mutation?: RefreshMutation
) => {
  const { post, root, postid, rootid } = args || {};
  const postOL = post?.querySelector(".oneline_body") as HTMLElement;
  const replyOL = root?.querySelector(
    `li#item_${mutation.postid} .oneline_body`
  ) as HTMLElement;
  ...
  ChromeShack.refreshing = [
    ...ChromeShack.refreshing.filter((r) => r.rootid !== args.rootid),
  ];
};
...
export const handleRootAdded = async (mutation: RefreshMutation) => {
  ...
  if (reply && root && !(await getEnabled("hide_tagging_buttons")))
    processReplyEvent.raise(raisedArgs, mutation);
  if (post && root)
...
export const handleReplyAdded = async (args: PostEventArgs) => {
  ...
  if (!disableTags && postRefreshBtn && shouldRefresh) postRefreshBtn.click();
    ...
    console.log(
      "too many replies in this thread - skipping nuLOL post reply fix"
    );
};
...
export const handleRefreshClick = async (e: MouseEvent) => {
  ...
  if (refreshBtn) {
    ...
    const rootRefreshBtn = root?.querySelector(
      "div.refresh > a"
    ) as HTMLElement;
    ...
    if (!disableTags && !is_root && shouldRefresh) {
      ...
      console.log(
        "too many replies in this thread - skipping nuLOL refresh fix"
      );
  }
};
...
export const contentScriptLoaded = async () => {
  ...
  const loggedInUsername =
    document.getElementById("user_posts")?.textContent || "";
...
export const processFullPosts = () => {
  const fullposts = [
    ...document.querySelectorAll("div.fullpost"),
  ] as HTMLElement[];
