      ...
      try {
        ...
        for (const mutation of mutationsList) {
          ...
          if (elemMatches(lastMutatedSibling, ".fullpost") && elemMatches(lastRemoved, ".inlinereply")) {
            ...
            const parentid = Number.parseInt(parent?.id?.substring(5), 10);
          ...
          if (mutation.type === "attributes" && mutation.oldValue && mutation.oldValue.indexOf("capped") > -1) {
            ...
            const rootid = root != undefined ? Number.parseInt(root.id?.substring(5), 10) : -1;
          ...
          for (const addedNode of mutation.addedNodes || []) {
            ...
            if (addedThread) {
              ...
              const rootid = Number.parseInt(addedThread?.id?.substring(5), 10);
