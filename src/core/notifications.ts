...
export const getEventId = async () =>
	(await getSetting("last_eventid")) as number;
export const setEventId = async (eventId: number) =>
	await setSetting("last_eventid", eventId);
...
export const TabMessenger = {
	send(msg: NotifyMsg) {
		...
		chrome.tabs
			.query({ url: "https://*.shacknews.com/chatty*" })
			.then((tabs) => {
...
const matchNotification = async (nEvent: NotifyEvent) => {
	...
	const postEventHasMatches =
		...
		matches?.reduce(
			(acc, m) => {
				...
				if (postEventBody.indexOf(mToLower) > -1 && !wasAdded)
					acc.push(m.trim());
				return acc;
			},
			[] as string[],
	...
	if (postEventHasMe) return "Someone mentioned your name.";
		...
		const message = `Someone mentioned: ${(
			postEventHasMatches as string[]
		).join(", ")}`;
		...
		if (event.eventType === "newPost") {
			...
			if (notify_enabled && match && post)
				chrome.notifications.create(
					`ChromeshackNotification${post?.id?.toString()}`,
					{
						type: "basic",
...
export const alarmNotifications = async () => {
	try {
		const enabled =
			(await getEnabled("enable_notifications")) ||
			(await getEnabled("highlight_pending_new_posts"));
		if (!enabled) return;
