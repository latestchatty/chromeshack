...
const VolumeUpIcon = () => (
	<FontAwesomeIcon className="unmute__icon" icon={faVolumeUp} />
);
const VolumeMuteIcon = () => (
	<FontAwesomeIcon className="mute__icon" icon={faVolumeMute} />
);

export const isVidPlaying = (v: HTMLVideoElement) =>
	!!(v.currentTime > 0 && !v.paused && !v.ended && v.readyState > 2);
...
const MuteOverlay = (props: OverlayProps) => {
	...
	return _predicate && audioEnabled ? (
		<div
			className="mute__overlay__container"
			onClick={onClick}
			title={visibility ? "Unmute" : "Mute"}
		>
...
const FlexVideo = (props: MediaProps) => {
	...
	const handleVideoState = () => {
		...
		const _hasAudio =
			vid && mozHasAudio ? mozHasAudio : !!(vid && wkAudioByteCount > 0);
	...
	return (
		<div
			className="media__boundary"
			style={isFirefox ? { position: "relative" } : undefined}
		>
			<MuteOverlay
