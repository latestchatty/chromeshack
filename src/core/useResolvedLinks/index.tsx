...
const loadComponent = (opts: URLProps) => {
	...
	if (type === "video" && src && /imgur/.test(src))
...
const resolveComponent = async (opts: URLProps): Promise<JSX.Element> => {
	...
	if (isValidElement(component) || isValidElement(resolved?.component))
	...
	const aResolved =
		arrHas(resolved) && resolved[0].src
			? (resolved as ParsedResponse[]).reduce((acc: JSX.Element[], r, i) => {
					const _component =
						r?.src &&
						loadComponent({ response: r, options, key: i.toString() });
					if (isValidElement(_component)) acc.push(_component);
					return acc;
			  }, [])
			: null;
	if (arrHas(aResolved) && aResolved.length > 1)
		return <Carousel slides={aResolved} />;
...
const resolveLink = async (opts: URLProps) => {
	...
	const resolved =
		detected && (await resolveComponent({ response: detected, options }));
	...
	const rComponent =
		pResolved?.component && isValidElement(pResolved.component)
			? pResolved.component
			: null;
	const lComponent = pResolved?.src
		? loadComponent({ response: pResolved, options })
		: null;
...
const resolveAlbum = async (opts: URLProps) => {
	...
	try {
		if (arrHas(links) && typeof links[0] !== "string")
		...
		if (arrHas(resolved) && resolved.length > 1)
			return <Carousel slides={resolved} />;
...
const resolveChildren = async (opts: URLProps) => {
	...
	const cResolved =
		response && (await resolveComponent({ links, response, options }));
