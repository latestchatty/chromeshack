...
const parseChattyPost = async (sanitizedFragment: DocumentFragment) => {
	const fullpost = sanitizedFragment.querySelector(
		".fullpost",
	) as HTMLDivElement;
	...
	const permalink = (
		sanitizedFragment.querySelector(".postnumber>a") as HTMLAnchorElement
	)?.href;
	const postid = parseInt(permalink?.split("item_")[1], 10);
	const author = (
		sanitizedFragment.querySelector(".user>a, .user") as HTMLAnchorElement
	)?.textContent;
	const saneAuthor = author?.replace(/\s/gm, "+");
	const icons = [
		...sanitizedFragment.querySelectorAll("span.author>img"),
	] as HTMLImageElement[];
	const postbody = (
		sanitizedFragment.querySelector(".postbody") as HTMLDivElement
	)?.innerHTML;
	const postdate = (
		sanitizedFragment.querySelector(".postdate") as HTMLDivElement
	)?.textContent;
...
const fetchChattyPost = async (postid: string) => {
	const singlePost =
		postid && `https://www.shacknews.com/frame_chatty.x?root=&id=${postid}`;
	if (singlePost) {
		...
		const sanitized = DOMPurify.sanitize(elemText, {
...
const Chattypost = (props: { parsed: ParsedChattyPost }) => {
	...
	const handleClick = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
		...
		const _link = (elemMatches(_this, "a") ||
			_this?.closest("a")) as HTMLAnchorElement;
		const spoilerTag =
			_this &&
			(elemMatches(
				_this,
				"div.chattypost__container span.jt_spoiler",
			) as HTMLSpanElement);
		if (_link?.href) {
			e?.preventDefault();
			...
			spoilerTag &&
			!spoilerTag.classList?.contains("jt_spoiler_clicked")
		) {
	...
	return objHas(parsed) ? (
		<div className="chattypost__container">
			<div
				className={`chattypost__hdr ${authorid ? `fpauthor_${authorid}` : ""}`}
			>
...
const parseLink = (href: string) => {
	const isChattyPost =
		/https?:\/\/(?:.+\.)?shacknews\.com\/chatty\?id=(\d+)/i.exec(href);
	return isChattyPost
		? ({
				href: isChattyPost[0],
				args: [isChattyPost[1]],
