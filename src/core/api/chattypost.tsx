...
const Chattypost = (props: { parsed: ParsedChattyPost }) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
    ...
    const spoilerTag = _this && (elemMatches(_this, "div.chattypost__container span.jt_spoiler") as HTMLSpanElement);
    if (_link?.href) {
      e?.preventDefault();
    ...
    <div className="chattypost__container">
      <div className={`chattypost__hdr ${authorid ? `fpauthor_${authorid}` : ""}`}>
        <div className="postnumber">
          <a rel="nofollow noreferrer" title="Permalink" href={permalink} onClick={handleClick}>{`#${postid}`}</a>
        </div>
