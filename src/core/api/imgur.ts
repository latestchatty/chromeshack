...
export const doResolveImgur = async ({
	imageId,
	albumId,
	galleryId,
}: ImgurResolution) => {
	try {
		const albumImageUrl =
			albumId && imageId
				? `${imgurApiAlbumBaseUrl}/${albumId}/image/${imageId}`
				: null;
		...
		const task = (
			await Promise.all(
				[albumImageUrl, albumUrl, imageUrl].map(async (u) => {
					...
					const album = arrHas(resp?.data?.images)
						? resp?.data?.images.map((i) => i?.mp4 || i?.link)
						: null;
		...
		throw new Error(
			`Could not resolve Imgur using any available method: ${imageId} ${albumId} ${galleryId}`,
		);
	} catch (e) {
		console.error(e);
		return null;
...
const parseLink = (href: string) => {
	...
	const albumMatch =
		/https?:\/\/(?:.+?\.)?imgur\.(?:com|io)\/(?:(?:album|gallery|a|g)\/(\w+)(?:#(\w+))?)/i.exec(
			href,
		);
	// galleryMatch[1] matches a tagged gallery nonce (which is actually an album)
	const galleryMatch =
		/https?:\/\/(?:.+?\.)?imgur\.(?:com|io)\/(?:t\/\w+\/(\w+))/i.exec(href);
	...
	const imageMatch =
		/https?:\/\/(?:.+?\.)?imgur\.(?:com|io)\/(?:i\/(\w+)|r\/\w+\/(\w+)|(\w+)\?(i|r)|(\w+))$/i.exec(
			href,
		);
	...
	const imageId = albumMatch
		? albumMatch[2]
		: imageMatch
		  ? imageMatch[1] || imageMatch[2] || imageMatch[3]
		  : null;
...
const doImgurUpload = async (
	data: UploadData,
	dispatch: Dispatch<UploaderAction>,
) => {
	try {
		...
		for (const file of data) {
			...
			const fileFormat =
				file instanceof File
					? matchFileFormat(file as File, imageFormats, videoFormats)
					: -1;
			...
			if (fileFormat === 0 || typeof file === "string")
				dataBody.append("image", file);
...
const handleImgurAlbumUpload = async (
	...
	if (!arrEmpty(hashes) && hashes?.length > 1) {
		...
		if (albumId) handleImgurSuccess([albumId], dispatch);
			...
			handleImgurFailure(
				{ code: 400, msg: "Something went wrong when creating album!" },
				dispatch,
			);
	} else if (hashes?.length === 1) handleImgurSuccess(links, dispatch);
	else
		handleImgurFailure(
			{ code: 400, msg: "Server returned no media links!" },
			dispatch,
		);
};

export const handleImgurUpload = async (
	data: UploadData,
	dispatch: Dispatch<UploaderAction>,
) => {
	try {
		...
		handleImgurFailure(
			{ code: 401, msg: e.message || "Something went wrong!" },
			dispatch,
		);
	}
};
