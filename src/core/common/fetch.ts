...
const parseChattypics = (html: string) => {
	...
	const _resElemArr = _resFragment.querySelector(
		"#allLinksDirect",
	) as HTMLInputElement;
	const _resElemVal = _resFragment.querySelector("#link11") as HTMLInputElement;
	const linksArr = _resElemArr.value
		.split("\n")
		.filter((x: string) => x !== "");
...
const parseShackRSS = (rssText: string) => {
	...
	if (rssText.startsWith('<?xml version="1.0" encoding="utf-8"?>')) {
		...
		for (const i of items || []) {
			...
			const content = i.match(
				/<description><!\[CDATA\[(.+)[\s\S]*?\]\]><\/description>/im,
			);
...
export const waitToResolve = (ms: number) =>
	new Promise((resolve) => setTimeout(resolve, ms));
export const waitToFetchSafe = async (
	timeout: number,
	fetchArgs: FetchArgs,
) => {
