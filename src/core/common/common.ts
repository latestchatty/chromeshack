...
export const arrHas = (arr: any[]) =>
	arr != null && Array.isArray(arr) && arr.length > 0;
export const arrEmpty = (arr: any[]) =>
	arr !== null && Array.isArray(arr) && arr.length === 0;
...
export const isImage = (href: string) =>
	/\.?(jpe?g|gif(?!v)|png|webp)/i.test(href);
...
export const getLinkType = (href: string) => {
	...
	const _isInstagram = /instagr\.am|instagram\./i.test(href)
		? "instagram"
		: null;
	const _isTwitter = /twitter\./i.test(href) ? "twitter" : null;
	const _isChattypost = /shacknews\.com\/chatty\?id=\d+/i.test(href)
		? "chattypost"
		: null;
	return (
		_isImage ||
...
export const isUrlArr = (dataArr: string[]) => {
	...
	for (const i of dataArr || [])
		if (typeof i !== "string" || i.length <= 9 || !i.match(/^https?:\/\//i))
			return false;
