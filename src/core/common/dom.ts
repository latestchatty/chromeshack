...
export const generatePreview = (postText: string) => {
	...
	for (const ix in complexReplacements) {
		const rgx = new RegExp(
			`${complexReplacements[ix].from[0]}(.*?)${complexReplacements[ix].from[1]}`,
			"g",
		);
		while (_postText.match(rgx) !== null)
			_postText = _postText.replace(
				rgx,
				`${complexReplacements[ix].to[0]}$1${complexReplacements[ix].to[1]}`,
			);
	}
...
export function scrollToElement(
	...
	if (_elem && _elem instanceof $)
		_elem = (_elem as JQuery<HTMLElement>)[0] as HTMLElement;
	else if (!_elem) return false;
	const headerHeight = -(
		document.querySelector("header")?.getBoundingClientRect().height + 6
	);
	...
	const scrollY =
		(_elem as HTMLElement).getBoundingClientRect().top +
		window.scrollY +
		visibleY;
...
export const scrollParentToChild = (
	...
	if (!isViewable)
		parent.scrollTop =
			childRect.top + parent.scrollTop - parentRect.top + _offset;
};
...
export const JSONToFormData = (jsonStr: string) => {
	...
	for (const key of Object.keys(_obj)) {
		const _file =
			_obj[key].data?.startsWith("data:") &&
			Base64ToFile(_obj[key].filename, _obj[key].data);
...
export const appendLinksToField = (
	field: HTMLInputElement,
	links: string[],
) => {
	/// try to append links to the bottom of text input field
	if (!field || field.value === undefined)
		return console.error("invalid field target:", field);
...
export const matchFileFormat = (
	...
	const _imgFormats =
		imgFormats && imgFormats.length > 0 ? imgFormats.split(",") : null;
	const _vidFormats =
		vidFormats && vidFormats.length > 0 ? vidFormats.split(",") : null;
	if (!_imgFormats && !_vidFormats) return -1;
	const _imgMatched =
		arrHas(_imgFormats) && _imgFormats.filter((fmt) => fmt === input.type);
	const _vidMatched =
		arrHas(_vidFormats) && _vidFormats.filter((fmt) => fmt === input.type);
...
export const locatePostRefs = (postElem: HTMLElement) => {
	...
	const post =
		elemMatches(_parent, "li.sel") ||
		(postElem?.closest && (postElem?.closest("li.sel") as HTMLElement));
	const root =
