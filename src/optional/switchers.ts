  ...
  async cacheSwitchers() {
    ...
    if (is_enabled && Switchers.resolved.length === 0) {
      ...
      const resolved = Switchers.offenders.reduce((acc, offender) => {
        const user =
          resolvedUsers[offender.new]?.[0] || resolvedUsers[offender.old]?.[0];
        const matchedOld =
          user && offender.old.toLowerCase() === user.username.toLowerCase();
        const matchedNew =
          user && offender.new.toLowerCase() === user.username.toLowerCase();
  ...
  async loadSwitchers(args: PostEventArgs) {
    ...
    if (is_enabled) {
      ...
      for (const offender of Switchers.resolved || []) {
        const offenderOLs = [
          ...(post?.querySelectorAll(`div.olauthor_${offender.id}`) ?? []),
        ] as HTMLElement[];
        const offenderFPs = [
          ...(post?.querySelectorAll(`div.fpauthor_${offender.id}`) ?? []),
        ] as HTMLElement[];
      ...
      for (const { posts, username, matched } of offenderMutations)
        for (const _post of posts)
          Switchers.rewritePost(_post, username, matched);
    }
  },
