...
const StepForwardIcon = memo(() => (
	<FontAwesomeIcon icon={faAngleDoubleRight} />
));
const CommentDotsIcon = memo(() => <FontAwesomeIcon icon={faCommentDots} />);

const ThreadPaneReply = memo(
	(props: { recent: ParsedReply; mostRecent?: boolean }) => {
		...
		return (
			...
			>
				<div className={classNames("cs_thread_pane_reply_arrow", `${mod}`)}>
					↪
				</div>
				...
				<div
					className={classNames(
						`cs_thread_pane_reply_author replyid_${authorid}`,
						{ op },
					)}
				>
					{author}
...
const ThreadPaneCard = memo((props: { post: ParsedPost }) => {
	...
	const { author, authorid, body, contained, count, mod, rootid } =
		localPost || {};
	...
	return localPost?.rootid ? (
		...
		>
			<div className="cs_thread_pane_card_header">
				<div className={`cs_thread_pane_root_author authorid_${authorid}`}>
					{author}
				</div>
				{contained && (
					<div
						className="cs_thread_contains_user"
						title="You replied to this thread"
					>
						<CommentDotsIcon />
					</div>
				)}
				<div className="cs_thread_pane_post_count">
					{count > 0 && `${count} posts`}
				</div>
				<div
					className="cs_thread_pane_reload"
					title="Refresh this thread"
					onClick={handleClickReload}
				>
					<RefreshIcon
						classes={classNames("refresh__icon", { loading: refreshed })}
					/>
				</div>
				<div
					className="cs_thread_pane_shortcut"
					title="Jump to thread"
					onClick={handleClickThreadShortcut}
				>
					<StepForwardIcon />
				</div>
			</div>
...
const ThreadPaneJumpToTop = () => {
	const handleJumpToTop = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
		...
		const csPane =
			_this?.closest && (_this?.closest("div#cs_thread_pane") as HTMLElement);
