...
const useThreadPaneCard = (post: ParsedPost) => {
	...
	const handleClickReload = useCallback(() => {
		const reloadBtn = document.querySelector(
			`.root#root_${rootid}>ul>li .refresh>a`,
		) as HTMLElement;
		if (reloadBtn) {
			reloadBtn.click();
			setRefreshed(true);
	...
	const updatePending = useCallback(
		(args: PendingPostEventArgs) => {
			...
			if (pending) return;
			const foundIdx = pendings?.findIndex(
				(p: PendingPost) => p.threadId === rootid,
			);
	...
	const userFilterUpdate = useCallback(
		(args: ResolvedUser) => {
			...
			(async () => {
				...
				const removeFullposts =
					cufEnabled && (await getEnabledSuboption("cuf_hide_fullposts"));
