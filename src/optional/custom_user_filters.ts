...
export const CustomUserFilters = {
	...
	async removeOLsForAuthorId({ id }: ResolvedUser, hideFPs: boolean) {
		...
		if (hideFPs)
			postElems = [
				...document.querySelectorAll(`div.olauthor_${id}, div.fpauthor_${id}`),
			];
		...
		for (const post of postElems || []) {
			...
			if (ol?.parentElement?.matches("li")) {
				...
				for (let i = children.length - 1; i > 0 && lastChild; i--) {
					...
					if (
						(hideFPs && !isChatty && !lastChildIsRoot) ||
						(!lastChildIsRoot && lastChild)
					)
	...
	async applyFilter() {
		...
		if (is_enabled) {
			...
			CustomUserFilters.rootPostCount =
				document.querySelector(".threads")?.childElementCount ?? 0;
