...
import { objEmpty, safeInnerHTML } from "../core/common";
...
const EmbedSocials = {
    ...
    processPost(e: MouseEvent, parsedPost: any, postId: string, index: number) {
        if (e.button == 0) {
            e.preventDefault();
    ...
    */
    async createTweet(parentLink: HTMLAnchorElement, socialId: string, postId: string, index: number) {
    ...
    async renderTweetObj(response: any) {
        const sortByBitrate = (mediaArr: any[]) => {
        ...
        const collectTweetMedia = (tweetMediaObj: Record<string, any>) => {
            const result = [] as any[];
    ...
    async fetchTweetParents(tweetObj: Record<string, any>) {
        const fetchParents = async (id: string, acc: TweetResponse[]): Promise<TweetResponse[]> => {
            ...
            return acc.reverse() as TweetResponse[];
        };
    ...
    renderTweet(parentLink: HTMLAnchorElement, tweetObj: Record<string, any>, postId: string, index: number) {
    ...
    compileTwitterMedia(
        parentLink: HTMLAnchorElement,
        templateElem: HTMLElement,
        tweetObj: Record<string, any>,
        postId: string,
        index: number,
    ) {
        const parseMedia = (mediaObjArr: any[]) => {
            const mediaArr = [] as any;
        ...
        if (!objEmpty(tweetObj.tweetMediaItems) && tweetObj.tweetMediaItems?.length > 0) {
