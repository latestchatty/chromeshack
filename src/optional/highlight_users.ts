...
export const HighlightUsers = {
  ...
  resolveUser(usernames: string[]) {
    ...
    if (Object.keys(HighlightUsers.cache).length === 0)
      HighlightUsers.resolveUsers();
  ...
  gatherCSS(users: ResolvedUsers, groups: HighlightGroup[]) {
    ...
    for (const user of usernames)
      for (const group of groups) {
        ...
        const foundUser = group.users?.find(
          (u) => u.toLowerCase() === user.toLowerCase()
        );

        if (group.name === "Original Poster")
          cssRules.push(
            `div.oneline.op span.oneline_user, .cs_thread_pane_reply_author.op { ${group.css} }`
          );
