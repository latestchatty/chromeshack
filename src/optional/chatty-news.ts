...
export const ChattyNews = {
  ...
  async checkTime(delayInMs: number) {
    const lastFetchTime = (await getSetting(
  ...
  async apply() {
    ...
    const articleChildren = [
      ...document.querySelectorAll(".article-content p:not(:nth-child(2))"),
    ];
    ...
    const articleBox = document.querySelector(
      ".article-content"
    ) as HTMLElement;
