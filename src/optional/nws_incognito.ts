...
export const NwsIncognito = {
    install() {
        ...
        processPostRefreshEvent.addHandler(NwsIncognito.hookToNwsPosts);
    },

    async hookToNwsPosts(args: PostEventArgs) {
        const { post } = args || {};
        const is_enabled = await enabledContains(["nws_incognito"]);
        if (is_enabled) {
            const nwsLinks = [...post?.querySelectorAll(":scope>.fpmod_nws > .postbody a")];
            for (const link of nwsLinks || [])
                await domMutate(() => {
                    ...
                    cloned.addEventListener("click", (e) => {
                        ...
                        browser.runtime
                            .sendMessage({ name: "allowedIncognitoAccess" })
