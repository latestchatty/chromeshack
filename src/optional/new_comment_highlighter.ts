...
export const NewCommentHighlighter = {
  ...
  async highlight(args?: PostEventArgs | void) {
    ...
    if (is_enabled) {
      const last_id = (await getSetting(
        "new_comment_highlighter_last_id",
        -1
      )) as number;
      const overTimeout = await NewCommentHighlighter.checkTime(
        NewCommentHighlighter.timeout
      );
      let new_last_id: number;
      if (!overTimeout) new_last_id = NewCommentHighlighter.findLastID(root);
      if (last_id > -1 && new_last_id >= last_id)
        await NewCommentHighlighter.highlightPostsAfter(last_id, root);
  ...
  async checkTime(delayInMs: number, reset?: boolean) {
    const now = Date.now();
    const lastHighlightTime = (await getSetting(
      "last_highlight_time",
      now
    )) as number;
    const overThresh = delayInMs
      ? timeOverThresh(lastHighlightTime, delayInMs)
      : now;
    if (reset || overThresh) {
  ...
  async highlightPostsAfter(last_id: number, root?: HTMLElement) {
    ...
    const oneliners = [
      ...(root || document).querySelectorAll("li[id^='item_']"),
    ];
    const process = async (li: HTMLElement, i: number, arr: any[]) => {
      ...
      const is_highlighted = preview?.classList?.contains(
      ...
      if (i === arr.length - 1) {
        ...
        const newComments =
          commentsCount && `${commentsCount} Comments (${newer.length} New)`;
  ...
  findLastID(root: HTMLElement) {
    ...
    // reply.
    const mostRecent = (root || document).querySelector(
      "div.oneline0"
    ) as HTMLElement;
