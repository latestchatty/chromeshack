...
const SaveIcon = () => (
	<FontAwesomeIcon className="templates__icon save--icon" icon={faCheck} />
);
const DelIcon = () => (
	<FontAwesomeIcon className="templates__icon del--icon" icon={faTimes} />
);
const AddIcon = () => (
	<FontAwesomeIcon className="templates__icon add--icon" icon={faPlus} />
);

const TemplateItem = memo(
	(props: { idx: number; body: string; onClick: any }) => {
		...
		return (
			<div className="template__item" data-idx={idx}>
				...
				<button
					...
					title="Save to this template slot"
					onClick={onClick}
				>
					<SaveIcon />
				</button>
...
const TemplatesApp = memo((props: { inputBox: HTMLInputElement }) => {
	...
	const saveTemplatesToStore = useCallback(() => {
		(async () => {
			...
			const _compressed = templates.map((t) =>
				t.length > 0 ? compressString(t) : "",
			);
			const areEqual =
				JSON.stringify(_templates) === JSON.stringify(_compressed);
			if (areEqual) return;
	...
	const handlePopupClick = useCallback(
		(e: React.MouseEvent<HTMLElement, MouseEvent>) => {
			...
			if (_text) {
				...
				setTemplates(
					templates.length > 1 ? templates.filter((_, i) => i !== _idx) : [""],
				);
			else if (_btn?.matches("#add__btn") && _hasBody)
				setTemplates([...templates, _val]);
		},
		[inputBox, templates],
	);
	...
	useEffect(() => {
		...
		if (height)
			popupRef.current.setAttribute("style", `top: -${height * 0.5}px;`);
	}, [popupVisible]);
	useEffect(() => {
	...
	useEffect(() => {
		const handleOutsideClick = (e: MouseEvent) => {
			...
			const isPopupChild =
				_this?.closest(".templates__popup") ||
				_this?.closest("#templates__btn");
	...
	return (
		<>
			<button
				id="templates__btn"
				className={classNames({ toggled: popupVisible })}
				onClick={handleBtnClick}
			>
				Templates
			</button>
			<div
				className={classNames("templates__popup", { visible: popupVisible })}
				ref={popupRef}
			>
				{templates.map((t, i) => {
					return (
						<TemplateItem key={i} idx={i} body={t} onClick={handlePopupClick} />
					);
				})}
			</div>
		</>
