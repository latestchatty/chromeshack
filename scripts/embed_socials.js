    ...
    if (getSetting("enabled_scripts").contains("embed_socials")) {
        ...
        EmbedSocials = {
            ...
            parseInstagram: function(parentLink, socialId, postId, index) {
                ...
                browser.runtime.sendMessage({ name: "corbFetch", url: postUrl, ovrBool: true })
                .then(response => {
                    var _matchGQL = response && response.gqlData.shortcode_media;
                    var _likes = response && response.likes;
                    ...
                    if (_matchGQL) {
                        ...
                        postPicDetails.innerText = _likes;
