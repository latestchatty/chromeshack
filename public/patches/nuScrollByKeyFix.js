...
function chat_onkeypress(b) {
	...
	const is_valid_target =
		sLastClickedItem !== -1 && sLastClickedRoot !== -1 && check_event_target(b);
	const a = String.fromCharCode(b.keyCode);
	const id =
		a === "Z"
			? get_item_number_from_item_string(
					get_next_item_for_root(sLastClickedRoot, sLastClickedItem),
			  )
			: a === "A"
			  ? get_item_number_from_item_string(
						get_prior_item_for_root(sLastClickedRoot, sLastClickedItem),
				  )
			  : false;
