...
test.describe("Image Uploader", () => {
  ...
  test("url detection and input state transition", async ({
    page,
    context,
  }) => {
    await navigate(page, url, undefined, context);
    ...
    const urlInputInvalid = await urlInput.evaluate(
      (e) => (e as HTMLInputElement).validity.patternMismatch
    );
    ...
    const urlInputValid = await urlInput.evaluate(
