...
test.describe("Postbox interactions", () => {
    test("Shacktags legend interactions", async ({ page, context }) => {
        ...
        const toggleBtn = page.locator("li li.sel #shacktags_legend_toggle");
    ...
    test("Drafts - input and persistence", async ({ page, context }) => {
        await navigate(page, url, { d: { enabled_scripts: ["drafts"], saved_drafts: [] } }, context);

        const replyBtn = page.locator("li li.sel div.reply>a");
        await replyBtn.click();
        const draftsDot = page.locator("li li.sel div.drafts__dot");
        await expect(draftsDot).toHaveClass(/invalid/);
    ...
    test("Templates - input and persistence", async ({ page, context }) => {
        await navigate(page, url, { d: { enabled_scripts: ["templates"], saved_templates: [] } }, context);
        ...
        await templatesBtn.click();
        page.locator("li li.sel div.template__item>span").nth(0).click();
        ...
        await templatesAddBtn.click({ clickCount: 2, delay: 250 });
    ...
    test("PostPreview complex interactions", async ({ page, context }) => {
        await navigate(page, url, { d: { enabled_scripts: ["post_preview", "drafts"], saved_drafts: [] } }, context);
    ...
    test("PostPreview - url detection", async ({ page, context }) => {
        await navigate(page, url, { d: { enabled_scripts: ["post_preview"] } }, context);
    ...
    test("PostPreview - codeblock formatting", async ({ page, context }) => {
        await navigate(page, url, { d: { enabled_scripts: ["post_preview"], tags_legend_toggled: true } }, context);
