...
test.describe("Settings Popup", () => {
  ...
  test("Test HGs CSS Interactions", async ({ page }) => {
    ...
    await expect(friendsInputBox).toHaveValue("border: 1px dotted white !important;");
    await expect(friendsCSSSplotch).toHaveCSS("border", "1px dotted rgb(255, 255, 255)");
    // change CSS and check for reactivity
    await friendsInputBox.fill("border: 1px dotted white !important; color: magenta;");
    await page.waitForTimeout(500);
